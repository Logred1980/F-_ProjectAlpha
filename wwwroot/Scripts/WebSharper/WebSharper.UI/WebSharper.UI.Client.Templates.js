...
export function InlineTemplate(el, fillWith){
      ...
      if(value[1].$==1){
        if(value[1].$1.$==1){
          if(value[1].$1.$1.$==1){
            if(value[1].$1.$1.$1.$==0){
              const s_1=value[0];
              _8=Dynamic(attrName, Map3((_9, _10, _11) => s_1+_9+_10+_11, value[1].$0, value[1].$1.$0, value[1].$1.$1.$0));
            }
            else {
              const s_2=value[0];
              _8=Dynamic(attrName, Map((vs) => s_2+concat("", vs), Sequence(value[1])));
            }
          }
          else {
            const s_3=value[0];
            _8=Dynamic(attrName, Map2((_9, _10) => s_3+_9+_10, value[1].$0, value[1].$1.$0));
          }
        }
        else if(value[0]=="")_8=Dynamic(attrName, value[1].$0);
        else {
          const s_4=value[0];
          _8=Dynamic(attrName, Map((v) => s_4+v, value[1].$0));
        }
      }
  ...
  return[_6, TreeReduce(Const(), Map2Unit, updates)];
}
